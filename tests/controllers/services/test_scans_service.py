...
import openscan.controllers.services.scans as scans
...
async def test_pause_scan_updates_status_and_persists(sample_scan_model: Scan) -> None:
    ...
    with patch("openscan.controllers.services.scans.get_task_manager", return_value=task_manager_mock), \
         patch("openscan.controllers.services.scans.get_project_manager", return_value=project_manager_mock):
...
async def test_pause_scan_without_task_id_returns_none(sample_scan_model: Scan) -> None:
    ...
    with patch("openscan.controllers.services.scans.get_task_manager") as tm_patch, \
         patch("openscan.controllers.services.scans.get_project_manager") as pm_patch:
        result = await scans.pause_scan(scan)
...
async def test_resume_scan_error_does_not_persist_state(sample_scan_model: Scan) -> None:
    ...
    with patch("openscan.controllers.services.scans.get_task_manager", return_value=task_manager_mock), \
         patch("openscan.controllers.services.scans.get_project_manager") as project_manager_patch:
